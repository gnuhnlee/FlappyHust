...
/* Definitions for myBuzzerTask */
osThreadId_t myBuzzerTaskHandle;
const osThreadAttr_t myBuzzerTask_attributes = {
  .name = "myBuzzerTask",
  .stack_size = 128 * 4,
  .priority = (osPriority_t) osPriorityNormal,
};
...
volatile uint8_t buzzer_state = BEEP_NONE;
osStatus_t r_state;
/* USER CODE END PV */
...
int main(void)
{
  ...
  /* creation of myBuzzerTask */
  myBuzzerTaskHandle = osThreadNew(StartBuzzerTask, NULL, &myBuzzerTask_attributes);
...
void Buzzer_SetFrequency(uint32_t freq)
{
    uint32_t timer_clk = 1000000;     // Sau Prescaler = 89
...
/**
* @brief Function implementing the myBuzzerTask thread.
* @param argument: Not used
* @retval None
*/
